...
import { CheckHwidBlockedQuery } from '@modules/hwid-user-devices/queries/check-hwid-blocked';
...
export class SubscriptionService {
    ...
    > {
        try {
            ...
            if (hwidHeaders !== null && (await this.checkHwidBlocklist(hwidHeaders.hwid))) {
                return this.buildSubscriptionHwidDeniedResponse(
                    user.response,
                    srrContext,
                    subscriptionSettings,
                    this.createHwidBlockedCheckup(),
                );
            }
    ...
    public async getRawSubscriptionByShortUuid(
        ...
        try {
            ...
            if (hwidHeaders !== null && (await this.checkHwidBlocklist(hwidHeaders.hwid))) {
                hwidCheckup = this.createHwidBlockedCheckup();
                headers['x-hwid-active'] = 'true';
                headers['x-hwid-blocked'] = 'true';
                headers['x-hwid-limit'] = 'true';
            } else if (patchedSettingEntity.hwidSettings.enabled) {
                const hwidCheckupResult = await this.checkHwidDeviceLimit(
                    user,
                    hwidHeaders,
            ...
            if (!hwidCheckup || hwidCheckup.subscriptionAllowed) {
                ...
                (hwidCheckup.maxDeviceReached ||
                    hwidCheckup.hwidNotSupported ||
                    hwidCheckup.hwidBlocked)
            ) {
                subscription = await this.renderTemplatesService.generateRawSubscription({
                    subscriptionSettings: patchedSettingEntity,
                    user,
                    ...
                    fallbackOptions: {
                        ...
                        showHwidBlockedRemarks: hwidCheckup.hwidBlocked,
                    },
                });
            }
    ...
    private createHwidBlockedCheckup(): IHwidCheckupResult {
        return {
            subscriptionAllowed: false,
            maxDeviceReached: false,
            hwidNotSupported: false,
    ...
    private async checkHwidBlocklist(hwid: string): Promise<boolean> {
        const blockedResult = await this.queryBus.execute(new CheckHwidBlockedQuery(hwid));

        if (!blockedResult.isOk) {
            this.logger.error(`Error checking HWID blocklist: ${blockedResult}`);
            return true;
        }

        return blockedResult.response.isBlocked;
    }

    private async buildSubscriptionHwidDeniedResponse(
        user: UserEntity,
        srrContext: ISRRContext,
        subscriptionSettings: SubscriptionSettingsEntity,
        hwidCheckup: IHwidCheckupResult,
        ...
        if (hwidCheckup.hwidBlocked) {
            response.headers['x-hwid-blocked'] = 'true';
        }
