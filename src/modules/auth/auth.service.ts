...
export class AuthService {
    ...
    constructor(
        ...
        @Optional()
        @Inject(forwardRef(() => DatabaseManagementGateService))
        private readonly databaseManagementGateService?: DatabaseManagementGateService,
        @Optional()
        @Inject(forwardRef(() => PostgresManagementGateService))
        private readonly postgresManagementGateService?: PostgresManagementGateService,
    ) {
    ...
    public async oauth2Callback(
        ...
        try {
            ...
            await this.databaseManagementGateService?.tryCompleteOAuthElevation(
                firstAdmin.response.uuid,
            );

            await this.postgresManagementGateService?.tryCompleteOAuthElevation(
                firstAdmin.response.uuid,
            );
