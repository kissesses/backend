...
@Injectable()
export class JwtDefaultGuard extends AuthGuard(['apiTokenJWT', 'registeredUserJWT']) {
    constructor(
    ...
    private async verifyApiToken(user: IJWTAuthPayload, apiTokenUuid: string): Promise<boolean> {
        ...
        if (!token.isOk) {
            await this.rawCacheService.del(`api:${apiTokenUuid}`);
            return false;
        }
        ...
        await this.rawCacheService.set(`api:${apiTokenUuid}`, scopes, 300);
        user.scopes = scopes;
        return true;
    }
}
