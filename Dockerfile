...
ARG BRANCH=main
ARG FRONTEND_URL=https://github.com/kissesses/frontend/releases/latest/download/remnawave-frontend.zip

RUN apk add --no-cache curl unzip ca-certificates \
    && curl -fL "${FRONTEND_URL}" -o frontend.zip \
...
FROM postgres:18.4 AS pg-tools
...
LABEL org.opencontainers.image.url="https://github.com/kissesses/backend"
LABEL org.opencontainers.image.source="https://github.com/kissesses/backend"
...
RUN apt-get update && apt-get install -y --no-install-recommends curl p7zip-full libpq5 && rm -rf /var/lib/apt/lists/*

COPY --from=pg-tools /usr/lib/postgresql/18/bin/pg_dump /usr/local/bin/pg_dump
COPY --from=pg-tools /usr/lib/postgresql/18/bin/pg_restore /usr/local/bin/pg_restore
